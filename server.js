...
const client = new Client({
    ...
    puppeteer: {
        executablePath: 'C:/Program Files/Google/Chrome/Application/chrome.exe' // Substitua pelo caminho do seu navegador
    }
});
...
client.on('qr', (qr) => {
    console.log('Nenhuma conta conectada. Escaneie o QR Code abaixo para conectar:');
...
function ensureClientReady(req, res, next) {
    if (!isClientReady) {
        return res.status(503).send({ error: 'Cliente do WhatsApp não está conectado. Aguarde a conexão.' });
    }
    next();
}
...
function formatPhoneNumber(number) {
    ...
    if (!/^\d{10,15}$/.test(cleaned)) {
        throw new Error('Número de telefone inválido. Certifique-se de usar o formato internacional, como 5511999999999.');
    }
